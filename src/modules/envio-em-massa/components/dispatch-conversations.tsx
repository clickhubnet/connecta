      ...
      <div className="wa-columns bg-card">
        <section aria-label="Conversas de disparos" className="wa-list-panel bg-card">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            ...
            {inbox.loading ? <p className="p-6 text-xs text-muted-foreground">Carregando…</p> : inbox.data?.conversations.map((item) => <button key={item.id} type="button" onClick={() => { setSelectedId(item.id); setNotice(""); }} className={`w-full border-b p-4 text-left hover:bg-muted/40 ${selectedId === item.id ? "bg-primary/5" : ""}`}><div className="flex items-center gap-2"><p className="min-w-0 flex-1 truncate text-sm font-semibold">{item.lead?.name ?? item.phone}</p>{item.pendingReplyCount > 0 && <span aria-label={`${item.pendingReplyCount} mensagens aguardando resposta`} title="Aguardando sua resposta" className="inline-flex h-5 min-w-5 shrink-0 items-center justify-center rounded-full bg-red-600 px-1.5 text-[11px] font-bold leading-none text-white">{item.pendingReplyCount > 99 ? "99+" : item.pendingReplyCount}</span>}{item.state === "BLOCKED" ? <span className="rounded-full bg-rose-100 px-2 py-0.5 text-[9px] font-semibold text-rose-700">Bloqueado</span> : null}</div><p className="mt-1 truncate text-xs text-muted-foreground">{messagePreview(item.messages[0]?.body) ?? "Sem mensagens"}</p><span className={`mt-2 inline-flex max-w-full items-center rounded-full border px-2.5 py-1 text-[10px] font-medium ${item.owner ? "border-primary/20 bg-primary/10 text-primary" : "border-border bg-muted/60 text-muted-foreground"}`}><span className="truncate">{item.owner?.name ?? "Sem responsável"}</span></span></button>)}
