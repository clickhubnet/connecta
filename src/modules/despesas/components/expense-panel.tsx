...
export function ExpensePanel() {
  ...
  return <div className="management-page space-y-5">
    <div className="management-toolbar flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><span className="management-heading-icon"><WalletCards className="h-5 w-5" /></span><div><h2 className="text-sm font-semibold">Controle de despesas</h2><p className="mt-1 text-xs text-muted-foreground">Organize pagamentos e acompanhe seus gastos.</p></div></div><Button onClick={() => setEditing(null)}><Plus className="h-4 w-4" />Cadastrar despesa</Button></div>
    {message && <p className="rounded-md border bg-card px-4 py-3 text-sm">{message}</p>}
    {resource.error && <p role="alert" className="text-sm text-destructive">{resource.error}</p>}
    <div className="grid gap-4 sm:grid-cols-3">
      <MetricCard title="Pendente" value={resource.loading ? "..." : currency.format(total("PENDING"))} helper="Contas aguardando pagamento" icon={Clock3} tone="amber" />
      <MetricCard title="Atrasado" value={resource.loading ? "..." : currency.format(total("OVERDUE"))} helper="Pagamentos que precisam de atenção" icon={AlertTriangle} tone="red" />
      <MetricCard title="Pago" value={resource.loading ? "..." : currency.format(total("PAID"))} helper="Pagamentos realizados" icon={CheckCircle2} tone="teal" />
    </div>
    <Card className="management-surface"><CardContent className="p-0">
    <div className="flex flex-col gap-3 border-b p-4 sm:flex-row sm:items-center"><div className="relative flex-1"><Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input aria-label="Pesquisar despesas" className="pl-9" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Pesquisar despesas..." /></div><select aria-label="Filtrar por status" className="h-10 rounded-xl border bg-background px-3 text-sm" value={filter} onChange={(event) => setFilter(event.target.value as ExpenseStatus | "")}><option value="">Todos os status</option><option value="PENDING">Pendente</option><option value="OVERDUE">Atrasado</option><option value="PAID">Pago</option></select><span className="text-xs text-muted-foreground tabular-nums">{filtered.length} registros</span></div>
    <div className="overflow-x-auto"><table className="management-table w-full min-w-[820px] text-left text-sm"><thead className="border-b text-xs text-muted-foreground"><tr><th className="px-4 py-3 font-medium">Despesa</th><th className="px-4 py-3 font-medium">Valor</th><th className="px-4 py-3 font-medium">Vencimento</th><th className="px-4 py-3 font-medium">Pagamento</th><th className="px-4 py-3 font-medium">Status</th><th className="px-4 py-3 text-right font-medium">Ações</th></tr></thead><tbody>
