...
async function main() {
  ...
  const plans = [
    { name: "Connecta Start", speed: "300 Mega", price: 99.9, order: 1 },
    { name: "Connecta Plus", speed: "500 Mega", price: 119.9, order: 2 },
    { name: "Connecta Ultra", speed: "700 Mega", price: 149.9, order: 3 },
  ];
  ...
  const adminEmail = process.env.ADMIN_INITIAL_EMAIL ?? "admin@connectatelecom.com.br";
  const adminPassword = process.env.ADMIN_INITIAL_PASSWORD ?? "Connecta@2026!";
  const adminPasswordHash = await hashPassword(adminPassword);

  await prisma.user.upsert({
    where: { email: adminEmail },
    create: {
      name: "Administrador Connecta",
      email: adminEmail,
